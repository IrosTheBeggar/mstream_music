...
| `android/autodj-multi-server.sh` | multi-server Auto DJ on two local servers scanned under the server's `test-fake` discovery model: a federated peer, the DJ hosted on that peer, then the same two as own servers, then a Quick Connect server that is only a fan-out candidate — its tunnel must dial when the session is armed and be released when the DJ is switched off; each phase expects a 2/2 fan-out pick that queues and plays. Needs a server checkout that reports ≥ 6.26.0 (header) | 5 min |
| `android/play-history.sh` | listening history end to end: a fresh local mStream 6.27+ and one account; three restored tracks played under adb — a counted skip, an uncounted skip, an airplane-mode play that drains on reconnect at its own start time, a force-kill recovered as `stopped` — on the phone's record and on the server, then the Listening page in both scopes. Needs a server checkout (header) | 4 min |
| `android/cast-through-rebuild.sh` | opt-in (`SMOKE_CAST=1`, plays on a TV): cast + tunnel disturbance | 2 min |
| `android/home-widget.sh` | opt-in (`SMOKE_WIDGET=1`, places a widget on the home screen): the Now Playing widget shows what Dart published, its Pause / Play / Next drive the handler, and a Play tap after a kill cold-boots the service headless. Debug build (the placement rides the `ext.mstream.widget` hook), English launcher | 2 min |
