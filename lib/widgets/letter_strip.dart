...
class _LetterStripState extends State<LetterStrip> {
  ...
  static Map<String, int> _buildIndex(List<DisplayItem> items) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      map.putIfAbsent(LetterStrip.indexLetter(items[i]), () => i);
    }
    return map;
  }
  ...
  Widget _verticalStrip() {
    return SizedBox(
      // 40 = hit region; the visible strip is stripWidth, pinned to whichever
      // edge the strip is on, leaving ~16px of extra reach on the list side.
      width: 40,
      child: Align(
