...
**Where the client lives now (2026-09-19):** the Rust core, its C ABI, the build scripts and the interop harness moved out of this repo into the shared crate <https://github.com/IrosTheBeggar/mstream-iroh-tunnel> (`rust/iroh_tunnel/` here is gone). This app loads the crate's prebuilt binaries from a tagged release, pinned by `tool/iroh-tunnel.properties` and the iOS `Package.swift` (moved with `tool/fetch-iroh-tunnel.sh <tag>`): the Android build downloads its `.so`'s, the iOS build resolves a SwiftPM remote binaryTarget, nothing is committed; the C ABI and the Dart binding are unchanged. The plan below is the history of building it. See `IROH_TUNNEL_CRATE_PLAN.md` for the move.
...
- ✅ **ABI build fixed:** `--split-per-abi` now works — `flutter build apk --flavor full --split-per-abi --target-platform android-arm,android-arm64,android-x64` builds per-ABI APKs (the arm64 slice is ~117 MB vs 240 MB universal). The manual `ndk.abiFilters` conflicted with the splits config AND was being silently overridden by the Flutter plugin in non-split builds; `disable-abi-filtering=true` (gradle.properties) makes our `ndk.abiFilters` authoritative and the filter is gated off for split builds.
- ✅ **armeabi-v7a kept for device coverage:** the native libs (`libiroh_tunnel.so` / `libprojectM-4.so` / `libvisualizer_bridge.so`) are arm64-v8a + x86_64 only, but `ndk.abiFilters` still ships an `armeabi-v7a` slice **without** them so 32-bit-ARM devices stay supported on Google Play — the visualizer reports "unavailable" and iroh servers are gated off there, the rest of the app runs. (Making the filter authoritative initially dropped armeabi-v7a from the AAB, which cut Play device support; re-adding the lib-free slice restores it.) The release guard asserts armeabi-v7a is present **and** lib-free.
- ✅ **Play camera + data safety:** decision = keep QR scan in the `play` build (camera kept, declared OPTIONAL via `uses-feature android.hardware.camera required="false"` so Play doesn't exclude camera-less devices; resolved the manifest TODOs). Play Console sensitive-permission declaration + Data-safety answers — including the iroh **relay-routing** disclosure (end-to-end-encrypted in transit; relay = transport intermediary, not data sharing) — written up in [`PLAY_COMPLIANCE.md`](PLAY_COMPLIANCE.md).
- Remaining: user docs (enable Remote Access on the server → scan/paste the pairing code in the app).
