...
- ✅ **ABI build fixed:** `--split-per-abi` now works — `flutter build apk --flavor full --split-per-abi --target-platform android-arm64,android-x64` yields a ~117 MB arm64 APK (vs 240 MB universal). The manual `ndk.abiFilters` conflicted with the splits config AND was being silently overridden by the Flutter plugin in non-split builds; `disable-abi-filtering=true` (gradle.properties) makes our `ndk.abiFilters` authoritative, the filter is gated off for split builds, and the release guard now also rejects armeabi-v7a — which had been leaking into the universal APK + Play AAB with no native libs (latent bug, now fixed).
